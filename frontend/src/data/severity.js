export const SEVERITY_META = {
  1: {
    ...
    intervention_options: ['1) Extend green cycle by 15s', '2) Update public advisory board', '3) Monitor only'],
  ...
  2: {
    ...
    intervention_options: ['1) Adaptive signal cycle (+30s)', '2) Reroute one bus line', '3) Public advisory push'],
  ...
  3: {
    ...
    intervention_options: ['1) Signal override + transit hold', '2) Divert traffic via ring road', '3) Two-lane closure + advisory'],
  ...
  4: {
    ...
    intervention_options: ['1) Full green-wave corridor + all cross-traffic hold', '2) Multi-lane closure + fire rescue dispatch', '3) Hospital pre-alert + ambulance pre-emption'],
  ...
  5: {
    ...
    intervention_options: ['1) Total corridor lockdown + green-wave to trauma center', '2) Mass diversion + multi-hospital pre-alert', '3) Full closure + fire/hazmat + BBMP debris crew'],
...
export function severityMeta(sev) {
  const n = Math.max(1, Math.min(5, parseInt(sev, 10) || 3))
  return SEVERITY_META[n]
}
